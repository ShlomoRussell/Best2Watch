...
const selectAllMovies = require("../dals");
...
movieDetailsCtrl.get("/", async function (req, res) {
    try {
        const movies = await selectAllMovies()
        res.send(movies);
...
movieDetailsCtrl.delete("/:movieId", function (req, res) {
    const isId = dal.deleteMovie(req.params.movieId);
    if (isId) res.sendStatus(200);
    else res.sendStatus(404);
});

movieDetailsCtrl.delete('/:movieId/:actor', function (req, res) {
    const isId = dal.deleteActor(req.params.movieId, req.params.actor);
    if (isId) res.sendStatus(200);
    else res.sendStatus(404);
})
